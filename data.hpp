...
#define NAME "aod_system"
    static MYSQL *MysqlInit()
    {
        ...
        if (mysql == NULL)
        {
            lg.LogMessage(Level::Error, "init mysql instance failed!\n");
            return NULL;
        }
        if (mysql_real_connect(mysql, HOST, USER, PASS, NAME, 0, NULL, 0) == NULL)
        {
            lg.LogMessage(Level::Error, "connect mysql server failed!\n");
            mysql_close(mysql);
            return NULL;
        }
    ...
    // 单数据库单表结构，一个mysql句柄就是一张表
    //数据库表结构 id  name电影名 info电影信息 video视频资源位置 image图片资源位置
    class TableVideo
    {
    private:
        MYSQL *_mysql;
