...
#define IMAGE_ROOT "/images/"
    ...
    private:
        ...
        static void Update(const httplib::Request &req, httplib::Response &rsp)
        {
            ...
            int video_id = std::stoi(req.matches[1]);
            Json::Value video;
            if (JsonUtils::Deserialize(req.body, &video) == false)
            {
                rsp.status = 400;
        ...
        static void SelectOne(const httplib::Request &req, httplib::Response &rsp)
        {
            ...
            // 3. 组织响应正文--json格式的字符串
            JsonUtils::Serialize(video, &rsp.body);
            rsp.set_header("Content-Type", "application/json");
            return;
        }
        static void SelectAll(const httplib::Request &req, httplib::Response &rsp)
        {
            ...
            if (req.has_param("search") == true)//看看是否输入了查询关键字
            {
            ...
            JsonUtils::Serialize(videos, &rsp.body);
            rsp.set_header("Content-Type", "application/json");
            return;
        }
    ...
    public:
        ...
        bool RunModule()
        {
            ...
            //	2. 添加请求-处理函数映射关系
            _srv.Post("/video", Insert);//restful post增加视频
